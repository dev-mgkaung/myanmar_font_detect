package mk.zawuni.zawgyiuni_detect.mmfont;
...
import mk.zawuni.zawgyiuni_detect.R;
...
public class MMFontUtils {
    ...
    private static void applyMMFontToMenuItem(MenuItem menuItem, Context context) {
        String mNewTitle = menuItem.getTitle().toString();
       // CustomTypefaceSpan typefaceSpan=null;
        if (mmTypeFace == null /*|| typefaceSpan == null*/ ) {
            mmTypeFace = Typeface.createFromAsset(context.getAssets(), "fonts/mymm.ttf");
            //typefaceSpan = new CustomTypefaceSpan("Arial Unicode MS", mmTypeFace);
        }
        if (!isSupportUnicode()) {
            mNewTitle = uni2zg(mNewTitle);
            SpannableStringBuilder spannableTitle = new SpannableStringBuilder(
                    MMFontUtils.mmText(mNewTitle, MMFontUtils.TEXT_UNICODE, true, true));
            spannableTitle.setSpan(mmTypeFace, 0, spannableTitle.length(), 0);
        ...
        for (int i = 0; i < menu.size(); i++) {
            ...
            if (subMenu != null && subMenu.size() > 0) {
                for (int j = 0; j < subMenu.size(); j++) {
                    ...
                    MMFontUtils.applyMMFontToMenuItem(subMenuItem,context);
                }
            }
            //the method we have create in activity
            MMFontUtils.applyMMFontToMenuItem(menuItem,context);
        }
    }
